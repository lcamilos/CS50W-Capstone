...
def student_regi(request):
    if request.method == "POST":
        ...
        if forms.is_valid():
            forms.save()
        messages.success(request, "Athlete Registration Successfully!")
...
def edit_student(request, pk):
    ...
    if request.method == "POST":
        ...
        if edit_forms.is_valid():
            edit_forms.save()
            messages.success(request, "Edit Athlete Info Successfully!")
            return redirect("student_list")
...
def delete_student(request, student_id):
    ...
    messages.success(request, "Delete Athlete Info Successfully")
    return redirect("student_list")
...
def attendance_count(request):
    ...
    if level_name:
        student_list = StudentInfo.objects.filter(level_type__level_short_form=level_name)
